...
row context about the group it belongs to, for example the mean income of
all rows in the same latitude band. Only feature columns are aggregated, so
the candidates carry no target information and cannot leak.
"""
...
def compute_bin_aggregates(
    ...
    the bin-level statistic of every column in ``agg_features``. Column
    names follow the pattern ``{bin_feature}bin{n_bins}_{stat}_{col}``.
    """
